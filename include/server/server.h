...
private:
    ...
    std::atomic<uint64_t> finalizeCpuUsTotal_{0};
    std::atomic<uint64_t> npuServiceUsTotal_{0};
    std::atomic<uint64_t> npuSlotWaitUsTotal_{0};
    std::atomic<uint64_t> layoutNpuServiceUsTotal_{0};
    std::atomic<uint64_t> layoutNpuSlotWaitUsTotal_{0};
    std::atomic<uint64_t> ocrOuterSlotHoldUsTotal_{0};
    std::atomic<uint64_t> ocrSubmoduleWindowUsTotal_{0};
    std::atomic<uint64_t> ocrSlotWaitUsTotal_{0};
    std::atomic<uint64_t> ocrCollectWaitUsTotal_{0};
    std::atomic<uint64_t> ocrInflightPeakTotal_{0};
    std::atomic<uint64_t> ocrBufferedOutOfOrderCountTotal_{0};
    std::atomic<uint64_t> tableNpuServiceUsTotal_{0};
    std::atomic<uint64_t> tableNpuSlotWaitUsTotal_{0};
    std::atomic<uint64_t> tableOcrServiceUsTotal_{0};
    std::atomic<uint64_t> tableOcrSlotWaitUsTotal_{0};
    std::atomic<uint64_t> cpuPreUsTotal_{0};
    std::atomic<uint64_t> cpuPostUsTotal_{0};
    std::atomic<uint64_t> tableFinalizeUsTotal_{0};
    std::atomic<uint64_t> ocrCollectOrMergeUsTotal_{0};
    std::atomic<uint64_t> layoutQueueWaitUsTotal_{0};
    std::atomic<uint64_t> planQueueWaitUsTotal_{0};
    std::atomic<uint64_t> ocrTableQueueWaitUsTotal_{0};
    std::atomic<uint64_t> finalizeQueueWaitUsTotal_{0};
    std::atomic<uint64_t> renderQueuePushBlockUsTotal_{0};
    std::atomic<uint64_t> layoutQueuePushBlockUsTotal_{0};
    std::atomic<uint64_t> planQueuePushBlockUsTotal_{0};
    std::atomic<uint64_t> ocrTableQueuePushBlockUsTotal_{0};
    std::atomic<uint64_t> queueBackpressureUsTotal_{0};
    std::atomic<uint64_t> pipelineOverlapFactorMilliTotal_{0};
    std::atomic<uint64_t> serialModeCount_{0};
    std::atomic<uint64_t> pagePipelineModeCount_{0};
